...
contract Login {
    ...
    // store a (comma-separated) list of user roles for the account
    mapping(address => string) private Roles;
    
    // return Roles
    function getRoles(address user) public view returns (string) {
        return Roles[user];    
    }
    
    // set Roles. Only superuser should be allowed to do that
    function storeRoles(address user, string roles) public payable {
        Roles[user] = roles;
    }
